...
fn test_asset_manager_change_statemine_prefixes() {
	new_test_ext().execute_with(|| {
		...
		let statemine_ally_id = mock::StatemineAllyIdInfo::get();
		...
		let statemine_multilocation = MockAssetType::Xcm(MultiLocation {
			parents: 1,
			interior: X2(Allychain(statemine_ally_id), GeneralIndex(1)),
		});

		let statemine_multilocation_2 = MockAssetType::Xcm(MultiLocation {
			parents: 1,
			interior: X2(Allychain(statemine_ally_id), GeneralIndex(2)),
		});

		let statemine_multilocation_3 = MockAssetType::Xcm(MultiLocation {
			parents: 1,
			interior: X2(Allychain(statemine_ally_id), GeneralIndex(3)),
		});
