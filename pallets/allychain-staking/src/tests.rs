...
fn round_immediately_jumps_if_current_duration_exceeds_new_blocks_per_round() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			roll_to(17);
			assert_last_event!(MetaEvent::AllychainStaking(Event::NewRound {
				starting_block: 10,
				round: 2,
...
fn set_allychain_bond_account_event_emits_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_last_event!(MetaEvent::AllychainStaking(
			Event::AllychainBondAccountSet { old: 0, new: 11 }
		));
	});
}
...
fn execute_leave_candidates_emits_event() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_last_event!(MetaEvent::AllychainStaking(Event::CandidateLeft {
				ex_candidate: 1,
				unlocked_amount: 10,
...
fn go_offline_event_emits_correctly() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_last_event!(MetaEvent::AllychainStaking(Event::CandidateWentOffline {
				candidate: 1
			}));
		});
}
...
fn go_online_event_emits_correctly() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_last_event!(MetaEvent::AllychainStaking(Event::CandidateBackOnline {
				candidate: 1
			}));
		});
}
...
fn delegator_bond_more_updates_candidate_state_top_delegations() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(AllychainStaking::top_delegations(1).unwrap().total, 10);
			assert_ok!(AllychainStaking::delegator_bond_more(
				Origin::signed(2),
				1,
				5
			...
			assert_eq!(AllychainStaking::top_delegations(1).unwrap().total, 15);
		});
}
...
fn delegator_bond_more_updates_candidate_state_bottom_delegations() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(AllychainStaking::bottom_delegations(1).unwrap().total, 10);
			assert_ok!(AllychainStaking::delegator_bond_more(
				Origin::signed(2),
				1,
				5
			...
			assert_eq!(AllychainStaking::bottom_delegations(1).unwrap().total, 15);
		});
}
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let mut new = vec![
				...
				Event::ReservedForAllychainBond {
					account: 11,
					value: 15,
				},
				Event::CollatorChosen {
...
fn delegation_kicked_from_bottom_removes_pending_request() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(AllychainStaking::delegate(Origin::signed(10), 1, 20, 8, 0));
			// check the event
			assert_event_emitted!(Event::DelegationKicked {
				delegator: 2,
				candidate: 1,
...
fn no_selected_candidates_defaults_to_last_round_collators() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let old_round = AllychainStaking::round().current;
			let old_selected_candidates = AllychainStaking::selected_candidates();
			...
			let new_round = AllychainStaking::round().current;
			// check AtStake matches previous
			let new_selected_candidates = AllychainStaking::selected_candidates();
