	...
	Runtime::Call: From<allychain_staking::Call<Runtime>>,
{
	...
	fn candidate_delegation_count(
		...
		gasometer.record_cost(RuntimeHelper::<Runtime>::db_read_gas_cost())?;
		let result =
			if let Some(state) = <allychain_staking::Pallet<Runtime>>::candidate_info(&address) {
	...
	fn execute_leave_candidates(
		...
		// Build call with origin.
		let origin = Runtime::AddressMapping::into_account_id(context.caller);
		let call = allychain_staking::Call::<Runtime>::execute_leave_candidates {
			candidate,
			candidate_delegation_count,
		};
	...
	fn execute_candidate_bond_less(
		...
		// Build call with origin.
		let origin = Runtime::AddressMapping::into_account_id(context.caller);
		let call = allychain_staking::Call::<Runtime>::execute_candidate_bond_less { candidate };
	...
	fn cancel_candidate_bond_less(
		...
		// Build call with origin.
		let origin = Runtime::AddressMapping::into_account_id(context.caller);
		let call = allychain_staking::Call::<Runtime>::cancel_candidate_bond_less {};
	...
	fn delegator_bond_more(
		...
		// Build call with origin.
		let origin = Runtime::AddressMapping::into_account_id(context.caller);
		let call = allychain_staking::Call::<Runtime>::delegator_bond_more { candidate, more };
