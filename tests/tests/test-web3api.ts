...
describeDevAxtend("Web3Api Information", (context) => {
  it("should include client version", async function () {
    ...
    let specVersion: string = await context.axiaApi.runtimeVersion.specVersion.toString();
    let implVersion: string = await context.axiaApi.runtimeVersion.implVersion.toString();
